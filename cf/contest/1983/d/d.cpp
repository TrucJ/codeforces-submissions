...
ll count(int a[], int temp[], int left, int right);
ll mergeAndCount(int a[], int temp[], int left, int mid, int right);
...
ll count(int a[], int temp[], int left, int right) {
    ...
    if (right > left) {
        ...
        inv_count += count(a, temp, left, mid);
        inv_count += count(a, temp, mid + 1, right);

        inv_count += mergeAndCount(a, temp, left, mid + 1, right);
    }
    return inv_count;
}
...
void solve()
{
    ...
    ll nt1 = countInversions(a, n);
    ll nt2 = countInversions(b, n);
