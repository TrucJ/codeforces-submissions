...
void solve()
{
    ...
    for (int i = 1; i <= n; i++)
    {
        cin >> a[i];
        f[i][1] = f[i-1][1] + (a[i] == 1);
        f[i][2] = f[i-1][2] + (f[i-1][1] + f[i-1][2]) * (a[i] == 2);
        f[i][3] = f[i-1][3] + f[i-1][2] * (a[i] == 3);
        for (int j = 1; j <= 3; j++) f[i][j] %= BASE_MOD;
    }
    cout << f[n][3];
}
...
// f(i,3) = f(i-1,2)
// f(i,2) = f(i-1,2) + [ f(i-1,1) if a[i] == 2 ]
// f(i,1) = f(i-1,1) + [ 1 if a[i] == 1 ]
// BASE_MOD
